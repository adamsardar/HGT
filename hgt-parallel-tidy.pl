...
my (
    %exclude, %parent, %genomes, %distances,
    %childs,  %stored, %results, %distribution
);
my ( @archs, @gens );
my $j            = 0;
my $iarch        = 0;
...
$sth = $dbh->prepare(
"SELECT DISTINCT len_comb.comb FROM len_comb WHERE $lengenquery AND len_comb.comb != '_gap_';"
);
$sth->execute();
while ( @temp = $sth->fetchrow_array() ) {
...
foreach $arch ( 0 .. scalar(@archs) - 1 ) {
    ...
    my $queryhandle    = $databasehandle->prepare(
"SELECT DISTINCT len_comb.genome FROM len_comb WHERE $lengenquery AND len_comb.comb = '$arch';"
    );
    $queryhandle->execute();
    ...
    until ( $flag == 0 ) {
        ...
        foreach $i ( keys(%genomes) ) {
            unless ( exists( $children{$i} ) ) {
                ...
                if ( $parent{$clade} eq '' ) {
                    die "Problem with the tree -$parent{$clade}- -$clade-\n";
                }
                $clade = $parent{$clade};
                last;
            }
